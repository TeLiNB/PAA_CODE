...
gene_list = ['gene1', 'gene2', 'gene3','gene4','gene5', 'gene6',
...
def data_split(filename):
    ...
    train_wine.to_csv('train_wine.csv', index=False)
    test_wine.to_csv('test_wine.csv', index=False)
...
def initial_groups( filename, target_tag, gene_list):
    ...
    gene10 = data['gene10'].tolist()
    
    gene11 = data['gene11'].tolist()
    
    gene12 = data['gene12'].tolist()
    
    gene13 = data['gene13'].tolist()
...
if os.path.exists('initial_group.npz') == 0:
    initial_group = initial_groups(filename, 2, gene_list)
...
if os.path.exists('normal_group.npz') == 0:
    normal_group = normal_groups(filename, 1,  gene_list)
...
# initial_group=initial_eliminate(normal_group, initial_group, gene_list)
# normal_group=normal_eliminate(normal_group, initial_group, gene_list)
...
discriminator_initial = train_gan(gan_initial, real_for_initial, initial_group, epochs=60, save_path='initial_gan_loss_curve.png')
...
for i in range(len(rate)):
    for j in range(len(generations)):
        ...
        with open(f'fitness_record1.csv', 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
        ...
        plot_dual_fitness_evolution(
            fitness_record,
            fitness_record1,
            save_path=f'dual_fitness_evolution.png'
)
        print('---------------------before_T_cell---------------------')
