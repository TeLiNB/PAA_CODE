...
def data_split( filename):
    ...
    train_wine = pd.DataFrame(columns=header)

    
    test_wine = pd.DataFrame(columns=header)
    ...
    for i in range(math.floor(len(Wine1) * 0.8)):
        train_wine = pd.concat([train_wine, data.iloc[[Wine1[i]]]])  

    for i in range(math.floor(len(Wine2) * 0.8)):
        train_wine = pd.concat([train_wine, data.iloc[[Wine2[i]]]])

    
    for i in range(math.floor(len(Wine1) * 0.2)):
        test_wine = pd.concat([test_wine, data.iloc[[Wine1[i + math.floor(len(Wine1) * 0.8)]]]])

    for i in range(math.floor(len(Wine2) * 0.2)):
        test_wine = pd.concat([test_wine, data.iloc[[Wine2[i + math.floor(len(Wine2) * 0.8)]]]])

    
    train_wine.to_csv('train_seeds.csv', index=False)
    test_wine.to_csv('test_seeds.csv', index=False)
...
def genetic_algorithm(initial_group, normal_group, gene_list, generations, rate):
    ...
    for generation in range(generations):
        print('-----------forward evolution', generation, '---------------------')
    ...
    for generation in range(generations):
        print('-----------backward evolution', generation, '---------------------')
...
def train_svm_classifier(population, population1,  id, rate, generations, test_size=0.2, random_state=42):
    ...
    with open(f'GAN_T_RBF_accuracy_records.csv', 'a', newline='', encoding='utf-8') as csvfile:
...
def plot_dual_fitness_evolution(fitness_record, fitness_record1, save_path='dual_fitness_evolution.png'):
    generations = list(range(1, len(fitness_record) + 1))
    
    plt.figure(figsize=(10, 6))
    plt.plot(generations, fitness_record, label='Abnormal Population (Forward)', color='red', linewidth=2)
    plt.plot(generations, fitness_record1, label='Normal Population (Reverse)', color='blue', linewidth=2)

    plt.title('Fitness Evolution Comparison: Normal vs Abnormal Populations', fontsize=14)
    ...
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"[INFO] Dual fitness evolution plot saved to {os.path.abspath(save_path)}")


data_split(filename)
train_file='train_wine.csv'



if os.path.exists('initial_group.npz') == 0:
    initial_group = initial_groups('train_seeds.csv', 2, gene_list)
...
if os.path.exists('normal_group.npz') == 0:
    normal_group = normal_groups( 'train_seeds.csv',1,  gene_list)
...
discriminator_initial = train_gan(gan_initial, real_for_initial, initial_group, epochs=30, save_path='D:/Desktop1/会议/seeds/initial_gan_loss_curve.png')
...
discriminator_normal = train_gan(gan_normal, real_for_normal, normal_group, epochs=50, save_path='D:/Desktop1/会议/seeds/normal_gan_loss_curve.png')
...
for i in range(len(rate)):
    for j in range(len(generations)):
        os.makedirs(f'{rate[i]}/{generations[j]}', exist_ok=True)
        ...
        plot_dual_fitness_evolution(
            fitness_record,
            fitness_record1,
            save_path=f'{rate[i]}/{generations[j]}/dual_fitness_evolution.png')
        ...
        # clf, X_test, y_test=train_svm_classifier(population, population1, 6, rate[i], generations[j], test_size=0.2, random_state=42)
