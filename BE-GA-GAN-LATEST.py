...
def data_split(filename):
    ...
    for i in range(math.floor(len(like) * 0.8)):
        train_iris = pd.concat([train_movie, data.iloc[[like[i]]]])  

    for i in range(math.floor(len(unlike) * 0.8)):
        train_iris = pd.concat([train_movie, data.iloc[[unlike[i]]]])

    
    for i in range(math.floor(len(like) * 0.2)):
        test_iris = pd.concat([test_movie, data.iloc[[like[i + math.floor(len(like) * 0.8)]]]])

    for i in range(math.floor(len(unlike) * 0.2)):
        test_iris = pd.concat([test_movie, data.iloc[[unlike[i + math.floor(len(unlike) * 0.8)]]]])

    
    train_iris.to_csv('train_movies.csv', index=False)
    test_iris.to_csv('test_movies.csv', index=False)


def data_collection(folder_path, rating, movie, gene_list):
   
    movies = pd.read_csv(folder_path + movie)


    
    ratings = pd.read_csv(folder_path + rating)
    ...
    data.to_csv('ml-latest.csv', index=False)
...
movie = 'train_ml-latest.csv'

rating = 'ratings.csv'
filename='ml-latest.csv'
data_split(filename)

# data_collection(folder_path, rating, movie, gene_list)

# feature = ['mean', 'var', 'stat.mode', 'max_min', 'kurtosis', 'skew']
    ...
    for j in range(len(generations)):
        ...
        with open('fitness_record1.csv', 'w', newline='', encoding='utf-8') as f: 
                ...
                for item in fitness_record1:
                    writer.writerow([item]) 
        print(f'---------------------before_T_cell---------------------')
        print("initial_population:", len(population))
        print("normal_population:", len(population1))

        population=initial_eliminate(population, discriminator_initial, init_prob)
        population1=normal_eliminate(population1, discriminator_normal, normal_prob)
        print('---------------------after_T_cell---------------------')
        print("initial_population:", len(population))
        print("normal_population:", len(population1))
